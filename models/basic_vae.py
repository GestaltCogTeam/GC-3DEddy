...
class Encoder(nn.Module):
    def __init__(
        ...
        # downsampling
        self.conv_in_sea = torch.nn.Conv2d(in_channels, self.ch, kernel_size=3, stride=1, padding=1)
        ...
        for i_level in range(self.num_resolutions):
            ...
            down.block = block
            down.attn = attn
            if self.num_resolutions-self.patch_size//2-2 < i_level < self.num_resolutions-1:
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            up.block = block
            up.attn = attn
            if 0 < i_level < self.patch_size//2+1:
                up.upsample = Upsample2x(block_in)
            self.up.insert(0, up)  # prepend to get consistent order
        
        # end
        self.norm_out = Normalize(block_in)
        self.conv_out_sea = torch.nn.Conv2d(block_in, in_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, z):
        ...
        # end
        h = self.conv_out_sea(F.silu(self.norm_out(h), inplace=True))
        return h
