...
class VQVAE(nn.Module):
    def __init__(
        ...
        ddconfig = dict(
            ...
            in_channels=vae_in_channels, ch_mult=(1, 1, 2, 2, 4), num_res_blocks=2,   # from vq-f16/config.yaml above
    ...
    def fhat_to_img(self, f_hat: torch.Tensor):
        return self.decoder(self.post_quant_conv(f_hat)).clamp_(-1.5, 1.5)
    ...
    def embed_to_img(self, ms_h_BChw: List[torch.Tensor], all_to_max_scale: bool, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
        if last_one:
            return self.decoder(self.post_quant_conv(self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=all_to_max_scale, last_one=True))).clamp_(-1.5, 1.5)
        else:
            return [self.decoder(self.post_quant_conv(f_hat)).clamp_(-1.5, 1.5) for f_hat in self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=all_to_max_scale, last_one=False)]
    ...
    def img_to_reconstructed_img(self, x, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None, last_one=False) -> List[torch.Tensor]:
        ...
        if last_one:
            return self.decoder(self.post_quant_conv(ls_f_hat_BChw[-1])).clamp_(-1.5, 1.5)
        else:
            return [self.decoder(self.post_quant_conv(f_hat)).clamp_(-1.5, 1.5) for f_hat in ls_f_hat_BChw]
