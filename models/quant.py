...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(
        ...
        # only used for progressive training of TAT (not supported yet, will be tested and supported in the future)
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                mean_vq_loss += F.mse_loss(f_hat.data, f_BChw, reduction='sum').mul_(self.beta) + F.mse_loss(f_hat, f_no_grad, reduction='sum')
    ...
    # ===================== idxBl_to_tat_input: only used in TAT training, for getting teacher-forcing input =====================
    def idxBl_to_tat_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
        next_scales = []
    ...
    # ===================== get_next_autoregressive_input: only used in TAT inference, for getting next step's input =====================
    def get_next_autoregressive_input(self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor) -> Tuple[Optional[torch.Tensor], torch.Tensor]: # only used in TAT inference
