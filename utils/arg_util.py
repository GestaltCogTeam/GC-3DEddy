...
class Args(Tap):
    ...
    # TAT
    tfast: int = 0      # torch.compile TAT; =0: not compile; 1: compile with 'reduce-overhead'; 2: compile with 'max-autotune'
    depth: int = 16     # TAT depth
    # TAT initialization
    ...
    data_load_channel: int = 7  # num of sea surface observations
    ...
    R_loss: float = None        # [automatically set; don't specify this]
    VQ_loss: float = None       # [automatically set; don't specify this]
    ...
    vR_loss: float = None       # [automatically set; don't specify this]
    vVQ_loss: float = None       # [automatically set; don't specify this]
    ...
    seed: int = None        # seed
    vae_if_train: bool = False
    vae_ckpt: str = 'vae_ch160v4096z32.pth'
    time_patch_num: int = 1
    train_val_test_ratio: str = '0.9_0.05_0.05'
    # get the even-numbered layers from the first twenty layers of GLORYS12V1
    vae_in_channels: int = 25
    vae_channels_list = list(range(0, 20, 2)) + list(range(20, 35))
...
def init_dist_and_get_args():
    ...
    args.train_val_test_ratio = [float(item) for item in args.train_val_test_ratio.split('_')]
    ...
    args.last_ckpt_path = os.path.join(args.local_out_dir_path, f'tat-ckpt-last.pth')
    _reg_valid_name = re.compile(r'[^\w\-+,.]')
    tb_name = _reg_valid_name.sub(
        '_',
        f'tb-TATd{args.depth}'
        f'__pn{args.pn}'
